...
class GmailService {
  constructor() {
    ...
    console.log('  - From Email:', process.env.EMAIL_FROM_EMAIL || process.env.GMAIL_USER);
    ...
    // Email configuration
    this.fromEmail = process.env.EMAIL_FROM_EMAIL || process.env.GMAIL_USER;
  ...
  async sendVerificationOTP(email, otpCode, userName = 'User') {
    console.log('📧 Starting verification OTP email send via Gmail...');
  ...
  async sendPasswordResetOTP(email, otpCode, userName = 'User') {
    console.log('🔒 Starting password reset OTP email send via Gmail...');
    console.log('  - To:', email);
    console.log('  - User:', userName);
    console.log('  - OTP:', otpCode);
