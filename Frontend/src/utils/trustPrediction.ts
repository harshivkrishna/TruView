...
const calculateContentQuality = (review: ReviewData): number => {
  ...
  const descText = review.description || '';
  ...
  const hasRelevantTag = reviewTags.some(tag => relevantTags.includes(tag));
...
const calculateEngagement = (review: ReviewData): number => {
  ...
  // Views score (0-0.4)
  if (views >= 100) score += 0.4;
  else if (views >= 50) score += 0.3;
  else if (views >= 20) score += 0.2;
  else if (views >= 5) score += 0.1;

  // Upvotes score (0-0.4)
  if (upvotes >= 20) score += 0.4;
  else if (upvotes >= 10) score += 0.3;
  else if (upvotes >= 5) score += 0.2;
  else if (upvotes >= 1) score += 0.1;

  // Engagement ratio (0-0.2)
  const engagementRatio = views > 0 ? upvotes / views : 0;
...
const calculateCredibility = (review: ReviewData): number => {
  ...
  const reviewTags = review.tags || [];
  if (reviewTags.includes('Honest')) score += 0.4;
  else if (reviewTags.includes('Brutal')) score += 0.3;
  else if (reviewTags.includes('Praise')) score += 0.2;
  else if (reviewTags.includes('Warning')) score += 0.2;
...
const calculateRecency = (review: ReviewData): number => {
  if (!review.createdAt) return 0.2;

  const now = new Date();
  const reviewDate = new Date(review.createdAt);

  // Handle invalid dates
  if (isNaN(reviewDate.getTime())) return 0.2;
